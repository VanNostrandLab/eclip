...
def barcoding(read, pattern=None, max_barcode_length=0, randomer_length=0, allow_mismatch=0):
    ...
    if pattern:
        matches = list(re.finditer(pattern, seq1[:max_barcode_length]))
        matches = [(k, len(v), 0) for k, v in matches[0].groupdict().items() if v] if matches else []
    else:
...
def fastd():
    ...
    max_barcode_length, n = max([len(v) for v in barcodes.values()]), 0
    ...
    for r1, r2 in zip(FastqGeneralIterator(opener(fastq1, 'r')), FastqGeneralIterator(opener(fastq2, 'r'))):
        barcode, read1, read2 = barcoding((r1, r2), pattern=pattern,
    ...
    if args.verbose:
        ...
        logger.debug(f'Successfully saved {n:,} paired reads.')
